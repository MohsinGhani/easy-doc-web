...
const MonthDayPicker: React.FC<MonthDayPickerProps> = ({
  ...
  const handleDaySelect = (day: Date | undefined) => {
    if (day && isDateWithinRange(day)) {
      const formattedDate = format(day, "do MMMM yyyy");
      setSelectedDay(day);
  ...
  const handleMonthChange = (month: Date) => {
    if (isDateWithinRange(month)) {
      setCurrentMonth(month);
    }
  };

  const MONTHS = next90Days
    .map((date) => ({
      value: date.getMonth(),
        ...
        <DayPicker
          ...
          components={{
            ...
            MonthCaption: ({ calendarMonth }) => {
              return (
                <div className="inline-flex gap-2 h-12 items-end">
                  <Select
